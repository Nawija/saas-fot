...
export default function GalleryPhotosPage() {
        ...
        if (link) {
            ...
            if (typeof window !== "undefined" && "history" in window) {
                window.history.replaceState(
                    null,
                    "",
                    `${window.location.pathname}#photo-${idx + 1}`
                );
            }
        }
    };
    ...
    useEffect(() => {
        ...
        // Event aktualizacji URL przy zmianie zdjęcia
        lightbox.on("change", () => {
        ...
        // jeśli hash w URL -> otwórz odpowiednie zdjęcie od razu
        const idxFromHash = getPhotoIndexFromHash(flatForHidden);
        if (idxFromHash !== null) {
            const link = orderContainer.querySelectorAll("a[data-photo-id]")[
                idxFromHash
            ] as HTMLAnchorElement | undefined;
